...
export class StdioRouter extends EventEmitter {
        ...
        const waitForInit = () => {
          return new Promise<void>((waitResolve, waitReject) => {
            ...
            initTimeout = setTimeout(() => {
              if (!initialized) {
                // Some servers may not send init messages
  ...
  async routeRequest(request: any): Promise<any> {
    ...
    this.logger.debug(`Current server connections:`, {
    ...
    this.logger.debug(`Selected target server: ${targetServer} for ${method}`, {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.logger.debug(`Sending request to ${targetServer}:`, {
  ...
  async routeToServer(serverName: string, request: any): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.logger.debug(`Sending targeted request to ${serverName}:`, {
        id: request.id,
  ...
  private async aggregateListResponses(method: string, params: any, id: number): Promise<any> {
    ...
    this.logger.debug(`Aggregating ${method} from ${connectedServers.length} connected servers`);
    ...
    if (method === 'tools/list') {
      ...
      this.logger.debug(`Aggregated ${allTools.length} tools total`);
    ...
    if (method === 'tools/call') {
      ...
      for (const [name, server] of this.upstreamServers) {
        if (toolName.startsWith(name + '__')) {
          this.logger.debug(`Matched tool ${toolName} to server ${name}`);
          return name;
        }
      }
  ...
  private handleUpstreamMessage(serverName: string, message: any): void {
    ...
    if (message.id !== undefined && message.id !== null) {
      ...
      if (this.pendingRequests.has(message.id)) {
        ...
        this.logger.debug(`Response received for request ${message.id} from ${serverName}`);
      ...
      if (message.method === '$/notification' && message.params) {
        ...
        this.logger.debug(`Upstream notification from ${serverName}: ${notificationMethod}`, {
