...
describe('MCPClient', () => {
  ...
  describe('switchRole', () => {
    it('should call set_role tool and parse manifest', async () => {
      const client = new MCPClient('node', ['server.js']);
      ...
      const switchPromise = client.switchRole('admin');
      ...
      const manifest = {
        ...
        availableTools: [],
        availableServers: [],
        metadata: { generatedAt: '2024-01-01', toolsChanged: true, toolCount: 0, serverCount: 0 }
      };
    ...
    it('should throw error when response has no content', async () => {
      ...
      await expect(switchPromise).rejects.toThrow('Failed to switch role');
    });
  });

  describe('listRoles', () => {
    it('should call set_role with list and parse result', async () => {
      const client = new MCPClient('node', ['server.js']);
      ...
      const rolesPromise = client.listRoles();

      const rolesResult = {
        roles: [
          { id: 'admin', name: 'Admin', description: 'Admin role', serverCount: 3, toolCount: 10, skills: ['admin-access'], isActive: true, isCurrent: true }
        ],
        currentRole: 'admin',
        defaultRole: 'guest'
      };
      ...
      setTimeout(() => {
        const response = JSON.stringify({
          ...
          id: 2,
          result: {
            content: [{ type: 'text', text: JSON.stringify(rolesResult) }]
          }
        });
        mockStdout.emit('data', response + '\n');
      }, 10);
      ...
      expect(result).toEqual(rolesResult);
    });
  });
